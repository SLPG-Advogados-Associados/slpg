...
import { RequisiteResult, RequisiteChain } from './engine/index'
...
const str = {
   ...
   */
  result: ({ from, to }: RequisiteResult) =>
    `${str.date(from)}^${str.date(to)}`,

  /**
   * String representation of multiple results.
   */
  results: (results: RequisiteResult[]) => results.map(str.result).join(', '),
